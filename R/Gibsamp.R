...
Gibbs(Y = swab$test_result1,X = X,groupID = swab$groupID,c = swab$age,alpha = 0.947,
  beta = 0.989,maxiter = 500,burn.in = 100)

GibbsInd = function(Y = NULL,X = X,c = c,grid = NULL,theta,theta0 = numeric(ncol(X)),
                    Sigma0 = length(Y) * solve(t(X) %*% X),m0 = 0,v0 = 0.burn.in = 2000) {
  ...
  Ispline <- function(x, order, knots) {
    ...
    yytem1 = yy1
    for (ii in 1:order) {
      yytem2 = array(rep(0, (n + k - 1 - ii) * length(x)), dim = c(n + k - 1 -
                                                                     ii, length(x)))
      for (i in (k - ii):n) {
        yytem2[i,] = (ii + 1) * ((x - t[i]) * yytem1[i,] + (t[i + ii + 1] - x) *
                                   yytem1[i + 1,]) / (t[i + ii + 1] - t[i]) / ii
      }
      yytem1 = yytem2
    }
    ...
    if (order == 1) {
      for (i in 2:n) {
        ibases[i - 1,] = (i < index - order + 1) + (i == index) * (t[i + order +
                                                                       1] - t[i]) * yytem2[i,] / (order + 1)
      }
    } else{
      for (j in 1:length(x)) {
        for (i in 2:n) {
          if (i < (index[j] - order + 1)) {
            ibases[i - 1, j] = 1
                     ...
                     (i >= (index[j] - order + 1))) {
            ibases[i - 1, j] = (t[(i + order + 1):(index[j] + order + 1)] - t[i:index[j]]) %*%
              yytem2[i:index[j], j] / (order + 1)
          } else{
            ibases[i - 1, j] = 0
          }
        }
  ...
  if (is.null(knots)) {
    k = 10
    knots = c(0, quantile(c, seq(0.1, 0.9, length.out = k - order)), max(c) +
                0.01)
  }
  if (is.null(grid)) {
    grid = knots
  ...
  I = Ispline(c, order, knots = knots)
  dij = numeric(N)
  zij = dij
  theta.mat = matrix(0, maxiter, k + p + 1)
  I1 = Ispline(grid, order, knots = knots1)
  ST = matrix(0, maxiter, length(grid))
  ...
  for (iter in 1:maxiter) {
    ...
    dij[Y == 1] = sapply(prob1[Y == 1], function(x) {
      rbinom(1, 1, x)
    })
    dij[Y == 0] = sapply(prob2[Y == 0], function(x) {
      rbinom(1, 1, x)
    })

    zij[dij == 0] = sapply(u[dij == 0], function(x) {
      rtruncnorm(1,-Inf, 0, x, 1)
    })
    zij[dij == 1] = sapply(u[dij == 1], function(x) {
      rtruncnorm(1, 0, Inf, x, 1)
    })
    ...
    #gam0 posterior
    W0 = v0 + N
    E0 = (v0 * m0 + sum(unlist(zij) - t(I) %*% gam - X %*% theta)) / W0
    gam0 = rnorm(1, E0, sqrt(1 / W0))
    ...
    #theta posterior
    Sigmatilde = solve(solve(Sigma0) + t(X) %*% X)
    thetatilde = c(Sigmatilde %*% (solve(Sigma0) %*% theta0 + t(X) %*% (zij -
                                                                          gam0 - t(gam %*% I))))
    theta = c(rmvnorm(1, thetatilde, Sigmatilde))
  ...
  #Throw away burn-in
  theta.mat = theta.mat[-(1:burn.in), 1:p]# HDI=apply(theta.mat[,1:p],2,hdi)
  ...
  return(
    list(
      ...
      q.025 = apply(theta.mat[, 1:p], 2, function(x) {
        quantile(x, 0.25)
      }),
      q.975 = apply(theta.mat[, 1:p], 2, function(x) {
        quantile(x, 0.975)
      }),
      grid = grid,
      St = colMeans(ST),
